...
async def list_users(
    ...
    # Dernière IP de connexion pour tous les utilisateurs de la page en une requête
    user_ids = [u.id for u in users]
    last_ips = {}
    if user_ids:
        login_logs = db.query(AuditLog.user_id, AuditLog.ip_address).filter(
            AuditLog.user_id.in_(user_ids),
            AuditLog.action == "login_success"
        ).order_by(desc(AuditLog.created_at)).all()
        for uid, ip in login_logs:
            if uid not in last_ips:
                last_ips[uid] = ip
