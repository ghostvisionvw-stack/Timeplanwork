...
# ── MON PROFIL ──
@router.get("/me")
async def get_profile(current_user: User = Depends(lambda credentials=Depends(HTTPBearer()), db=Depends(get_db): get_current_user(credentials, db))):
    return {
...
async def update_profile(
    ...
    request: Request,
    current_user: User = Depends(lambda credentials=Depends(HTTPBearer()), db=Depends(get_db): get_current_user(credentials, db)),
