...
def _ascii_filename(name: str) -> str:
    """Nettoie un nom pour l'en-tête Content-Disposition (latin-1/ASCII uniquement,
    sinon Starlette lève une erreur d'encodage → HTTP 500)."""
    normalized = unicodedata.normalize("NFKD", name).encode("ascii", "ignore").decode("ascii")
    return re.sub(r"[^A-Za-z0-9_-]", "_", normalized) or "utilisateur"
...
class WorkDayCreate(BaseModel):
    ...
    def time_valid(cls, v):
        if v is not None and v != "" and not TIME_RE.match(v):
            raise ValueError("Heure invalide (format attendu : HH:MM).")
        return v or None

    @field_validator("real_minutes", "paid_minutes", "gap_minutes")
    @classmethod
    def minutes_valid(cls, v):
        if v < -1440 or v > 2880:
            raise ValueError("Durée hors limites.")
        return v
...
class WorkDayBulk(BaseModel):
    ...
    @classmethod
    def days_limit(cls, v):
        if len(v) > 1000:
            raise ValueError("Trop de journées en une seule synchronisation (max 1000).")
        return v
...
async def sync_workdays(
    ...
    synced = 0
    # Une seule requête pour toutes les journées existantes (au lieu d'une par jour)
    existing_by_date = {
        wd.date: wd for wd in db.query(WorkDay).filter(WorkDay.user_id == current_user.id).all()
    }
    for day in body.days:
        ...
        if existing:
            ...
            db.add(new_day)
            existing_by_date[day.date] = new_day
        synced += 1
    db.commit()
...
async def export_pdf(
    ...
    if mode == 'monthly':
        ...
        nom = _ascii_filename(current_user.full_name or 'utilisateur')
